...
INSERT INTO import_receipts VALUES(1,'INIT-000001',1,0,'COMPLETED',NULL,4,2,'2026-07-01 08:00:00','2026-07-01 08:30:00');
...
WITH RECURSIVE seq(n)AS(SELECT 1 UNION ALL SELECT n+1 FROM seq WHERE n<100),loc(pid,locs)AS(SELECT 1,JSON_ARRAY(1,2)UNION ALL SELECT 2,JSON_ARRAY(1,2)UNION ALL SELECT 3,JSON_ARRAY(3,4)UNION ALL SELECT 4,JSON_ARRAY(3,4)UNION ALL SELECT 5,JSON_ARRAY(5)UNION ALL SELECT 6,JSON_ARRAY(6)UNION ALL SELECT 7,JSON_ARRAY(7)UNION ALL SELECT 8,JSON_ARRAY(8)UNION ALL SELECT 9,JSON_ARRAY(9,10)UNION ALL SELECT 10,JSON_ARRAY(9,10)UNION ALL SELECT 11,JSON_ARRAY(11)UNION ALL SELECT 12,JSON_ARRAY(14,15)UNION ALL SELECT 13,JSON_ARRAY(14,15)UNION ALL SELECT 14,JSON_ARRAY(16)UNION ALL SELECT 15,JSON_ARRAY(17)UNION ALL SELECT 16,JSON_ARRAY(18)UNION ALL SELECT 17,JSON_ARRAY(19)UNION ALL SELECT 18,JSON_ARRAY(20)UNION ALL SELECT 19,JSON_ARRAY(21)UNION ALL SELECT 20,JSON_ARRAY(22)UNION ALL SELECT 21,JSON_ARRAY(23)UNION ALL SELECT 22,JSON_ARRAY(24)UNION ALL SELECT 23,JSON_ARRAY(25)UNION ALL SELECT 24,JSON_ARRAY(26)UNION ALL SELECT 25,JSON_ARRAY(27)UNION ALL SELECT 26,JSON_ARRAY(28)UNION ALL SELECT 27,JSON_ARRAY(29)UNION ALL SELECT 28,JSON_ARRAY(12)UNION ALL SELECT 29,JSON_ARRAY(13)UNION ALL SELECT 30,JSON_ARRAY(30)UNION ALL SELECT 31,JSON_ARRAY(27,28))
SELECT CONCAT('INIT-',i.id,'-',LPAD(ROW_NUMBER()OVER(PARTITION BY i.product_id ORDER BY s.n),3,'0')),i.product_id,'SERIALIZED',i.id,CAST(JSON_UNQUOTE(JSON_EXTRACT(loc.locs,CONCAT('$[',(s.n-1)%JSON_LENGTH(loc.locs),']')))AS UNSIGNED),'IN_STOCK','2026-07-01 08:00:00',i.warranty_months
FROM import_receipt_items i JOIN seq s ON s.n<=i.quantity JOIN loc ON loc.pid=i.product_id ORDER BY i.product_id,s.n;

INSERT INTO export_receipts VALUES(1,'EXP-20260702-001','SALE',1,58992000,'COMPLETED',NULL,4,2,'2026-07-02 14:00:00','2026-07-02 16:00:00');
...
INSERT INTO export_receipts VALUES(2,'EXP-20260708-001','SALE',3,30598000,'PENDING_APPROVAL','Chờ duyệt xuất',3,NULL,'2026-07-08 11:00:00','2026-07-08 11:00:00');
