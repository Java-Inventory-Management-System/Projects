import type {
  ResponsePage, SupplierResponse, LocationResponse, CustomerResponse,
  ProductResponse, ImportReceipt, ImportReceiptItem,
  ExportReceipt, ExportReceiptItem, StockCheckStatus,
  StockCheck, StockCheckItem, StockAdjustment,
  PriceAdjustment, InventorySummary, LowStockItem,
  CategoryStock, StockValueItem, ActivityItem,
  DeadStockItem, ProductUnit, ProductImage, PurchaseOrderItem, PurchaseOrder,
} from "@/utils/types"
...
export function mapLocation(raw: unknown): LocationResponse {
  ...
  return { id: r.id, zoneCode: r.zoneCode, shelfCode: r.shelfCode, binCode: r.binCode, fullCode: r.fullCode, description: r.description ?? null, isActive: r.isActive ?? true, maxCapacity: r.maxCapacity ?? null, createdAt: r.createdAt ?? "", updatedAt: r.updatedAt ?? "" }
}

export function mapCustomer(raw: unknown): CustomerResponse {
  const r = raw as CustomerResponse
  return { id: r.id, name: r.name, phone: r.phone ?? null, email: r.email ?? null, address: r.address ?? null, note: r.note ?? null, isActive: r.isActive ?? true, createdAt: r.createdAt, updatedAt: r.updatedAt ?? "" }
}
...
export function mapImportItem(raw: unknown): ImportReceiptItem {
  ...
  return { id: r.id, productId: r.productId, productName: r.productName, productSku: r.productSku, quantity: r.quantity, unitPrice: r.unitPrice, warrantyMonths: r.warrantyMonths, createdUnits: r.createdUnits }
}
...
export function mapExportItem(raw: unknown): ExportReceiptItem {
  ...
  return { id: r.id, productId: r.productId, productName: r.productName, productSku: r.productSku, quantity: r.quantity, unitPrice: r.unitPrice }
}
...
export function mapActivityItem(raw: unknown): ActivityItem {
  ...
  return { type: r.type as ActivityItem["type"], receiptCode: r.receiptCode, date: r.date, counterpartyName: r.counterpartyName ?? null, lineItems: r.lineItems, totalAmount: r.totalAmount }
}
