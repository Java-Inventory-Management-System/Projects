    ...
    <CardContent className="p-0">
      {isLoading ? (
        ...
        <>
          ...
          <p
            className={`mt-1 text-2xl font-semibold tracking-tight tabular-nums ${highlight ? "text-destructive" : ""}`}
          >
            {value}
          </p>
        </>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StatCard
          label="Giá trị tồn"
          value={summary?.totalStockValue != null ? `${summary.totalStockValue.toLocaleString("vi-VN")}₫` : "—"}
          isLoading={isLoading}
        />
        <StatCard
          label="Sắp hết"
          ...
          isLoading={isLoading}
          highlight={!!summary?.lowStockCount}
        />
        <StatCard
          label="Hết hàng"
          value={String(summary?.outOfStockCount ?? "—")}
          isLoading={isLoading}
          highlight={!!summary?.outOfStockCount}
        />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
                ...
                <BarChart data={barGroupData}>
                  ...
                  <ChartTooltip
                    ...
                    content={
                      <ChartTooltipContent
                        hideLabel
                        formatter={(v: unknown) => (v as number).toLocaleString("vi-VN") + "₫"}
                      />
                    }
                  />
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Phân bổ sản phẩm theo danh mục</CardTitle>
          </CardHeader>
          <CardContent>
            {!categories ? (
              <Skeleton className="h-64 w-full" />
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Top sản phẩm theo giá trị tồn</CardTitle>
          </CardHeader>
          <CardContent>
            {!stockValue ? (
              <Skeleton className="h-64 w-full" />
