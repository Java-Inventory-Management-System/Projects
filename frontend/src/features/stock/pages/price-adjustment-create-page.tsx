            ...
            <CardContent className="pt-4">
              <div className="flex items-center justify-center gap-4">
                <div className="text-center">
                  ...
                  <p className="text-lg font-semibold text-muted-foreground">{oldPrice.toLocaleString("vi-VN")}₫</p>
                </div>
              ...
              {Number(newPrice) > 0 && (
                <div className="mt-2 text-center">
                  <span className={`text-xs font-medium ${Number(newPrice) > oldPrice ? "text-destructive" : "text-green-600"}`}>
                    {Number(newPrice) > oldPrice ? "Tăng" : "Giảm"} {Math.abs(((Number(newPrice) - oldPrice) / oldPrice) * 100).toFixed(1)}%
                  </span>
                </div>
              )}
            </CardContent>
