...
export async function createLocation(data: { zoneCode: string; shelfCode: string; binCode: string; description?: string }): Promise<LocationResponse> {
  const res = await http.post("/location", data)
  return mapLocation(res)
}

export async function updateLocation(id: number, data: { zoneCode: string; shelfCode: string; binCode: string; description?: string }): Promise<LocationResponse> {
