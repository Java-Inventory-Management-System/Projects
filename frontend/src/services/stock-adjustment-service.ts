...
export async function getStockAdjustments(page = 0, size = 20, type?: string, status?: string): Promise<ResponsePage<StockAdjustment>> {
...
export async function getMyStockAdjustments(page = 0, size = 20, type?: string, status?: string): Promise<ResponsePage<StockAdjustment>> {
  const params: Record<string, string | number> = { page, size, sort: "createdAt,desc" }
  if (type) params.type = type
  if (status) params.status = status
