...
- Nguồn: `import_receipts`, mục 5.1 bước 1.
- Priority: Must.
...
- Nguồn: `import_receipt_items`, mục 5.1 bước 2.
- Priority: Must.
...
- Nguồn: `product_units.serial_number UK`, mục 5.1 bước 3, mục 5 (edge case "Import serial file lỗi 1 phần").
- Priority: Must.
...
- Nguồn: `locations`, mục 5.1 bước 4.
- Priority: Should.
...
- AC: Thao tác trong 1 transaction; `imported_at` set tại thời điểm xác nhận (không phải lúc tạo record nháp); phiếu chuyển `completed`; ghi audit log #1.
- Nguồn: mục 5.1 bước 6, mục 2 (FIFO milestone), mục 7.2 bảng audit #1.
- Priority: Must.
...
- Nguồn: mục 5.1 bước 7, mục 7.2 audit #2, mục 5 (edge case "Nhập sai serial sau xác nhận").
- Priority: Should.
...
- Nguồn: `export_receipts.reason`, mục 5.2 bước 1.
- Priority: Must.
...
- Nguồn: mục 5.2 bước 3, mục 3 (FIFO tự động).
- Priority: Must.
...
- Nguồn: mục 5.2 bước 2, mục 5 (edge case "Xuất không đủ hàng").
- Priority: Must.
...
**US-11** | Là **NV**, tôi muốn xác nhận phiếu xuất, để chuyển trạng thái các unit sang `sold` và kích hoạt bảo hành nếu là bán hàng.

- AC: Trong 1 transaction: `product_units.status → sold`; nếu `reason = sale` → set `warranty_start_date` = ngày xuất, tính `warranty_expires_at`; phiếu `completed`; ghi audit log #4.
- Nguồn: mục 5.2 bước 4, mục 7.2 audit #4.
- Priority: Must.
...
- Nguồn: mục 5.3 bước 1, mục 5 (edge case "Ký tự serial gây nhầm lẫn").
- Priority: Must.
...
- Nguồn: `warranty_requests`, mục 5.3 bước 2-3.
- Priority: Must.
...
- Nguồn: mục 5.3 bước 4, mục 7.5 (Warranty Inheritance), mục 2 (state machine).
- Priority: Must.
...
- Nguồn: mục 5.3 bước 5, mục 7.2 audit #9.
- Priority: Must.
...
- Nguồn: `stock_adjustments`, mục 5.4 bước 1-2.
- Priority: Must.
...
- AC: **Người duyệt bắt buộc khác người tạo** — không ai được tự duyệt phiếu mình tạo (kể cả Admin).
- Nguồn: mục 5.4 bước 4, mục 7.2 audit #7, mục 6 (phân quyền), US-42.
- Priority: Must.
...
- AC: AD **không thể tự gán role ADMIN cho chính mình**. AD không thể gán role ADMIN cho user khác (chỉ seed data tạo ADMIN đầu tiên).
...
### 8.12 Epic 12 — 4-eyes Principle & Separation of Duties

Phát sinh từ review SAD: Admin toàn quyền trong thiết kế gốc (section 6), nhưng thực tế admin cũng là nhân viên được thuê — cần cơ chế kiểm soát để tránh xung đột lợi ích.

**US-42** | Là **HT**, tôi muốn enforce invariant `created_by != approved_by` cho tất cả hành động cần duyệt, để đảm bảo không ai tự duyệt phiếu do mình tạo.

- AC: Áp dụng cho: điều chỉnh tồn thủ công (US-20), duyệt kiểm kê lệch (US-24), duyệt phiếu nhập (nếu có bước duyệt).
- AC: Enforce ở Service layer — throw `PermissionDeniedException` nếu `created_by == approved_by`.
- AC: Kiểm tra bằng ArchUnit (quét các service method có `@AuditLog` + tham số `approved_by`) + integration test.
- AC: Không áp dụng cho các hành động một người (view, tạo nháp, tra cứu).
- Nguồn: mục 5.4 bước 4, mục 6 (phân quyền — cập nhật), section 6 note.
- Priority: Must.
